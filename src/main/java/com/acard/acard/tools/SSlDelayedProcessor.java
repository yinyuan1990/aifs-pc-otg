 ...
 */
public class SSlDelayedProcessor {
    ...
    private final ScheduledExecutorService delayedExecutor = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r, "Ssl-Delayed-Processor");
        t.setDaemon(true);
        return t;
    });
    ...
    private final ScheduledExecutorService backgroundExecutor = Executors.newScheduledThreadPool(2, r -> {
        Thread t = new Thread(r, "Ssl-Background-Worker");
        t.setDaemon(true);
        return t;
    });

    // 单例
    private static final SSlDelayedProcessor INSTANCE = new SSlDelayedProcessor();

    private SSlDelayedProcessor() {
    }

    public static SSlDelayedProcessor getInstance() {
        return INSTANCE;
    }
    ...
    public void addSlowEvent(CaptureEvent event) {
        if (event == null || event.getType() != CaptureEvent.Type.REALTIME) {
            return;
        }

        slowList.add(event);
        System.out.println("📌 添加Ssl事件到延迟队列: " + event.getEventId().substring(0, 8) +
    ...
    private void scheduleDelayedProcessing(CaptureEvent event) {
        delayedExecutor.schedule(() -> {
            ...
            System.out.println("⏰ 开始延迟处理Ssl事件: " + event.getEventId().substring(0, 8));
    ...
    private void processSlowEventBatch(CaptureEvent event) {
        try {
            ...
            System.out.println("🎉 Ssl事件批量处理完成: " + event.getEventId().substring(0, 8) +
                    ", 处理了 " + allPostFrames.size() + " 帧");

        } catch (Throwable e) {
            System.err.println("❌ 批量处理Ssl事件失败: " + event.getEventId().substring(0, 8) +
                    ", 错误: " + e.getMessage());
            e.printStackTrace();
            removeEventFromList(event);
        }
    ...
    private List<DiskCaptureCache.DiskFrameItem> batchGetAllPostFrames(CaptureEvent event) {
        ...
        try {
            File slowDir = new File("runtime/captures/ssl");
            if (!slowDir.exists()) {
                System.out.println("⚠️ Ssl目录不存在: " + slowDir.getAbsolutePath());
                return sourceFrames;
            }
