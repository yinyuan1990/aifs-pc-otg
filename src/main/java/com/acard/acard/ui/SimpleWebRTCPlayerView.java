...
public class SimpleWebRTCPlayerView extends StackPane {
    ...
    // 慢放缓存图像保存参数（⭐ 低端机优化：降低质量和分辨率）
    private static final boolean SLOW_SAVE_AS_JPEG = true; // 使用JPEG以减小体积提升写盘速度
    private static final float SLOW_JPEG_QUALITY = 0.80f; // 0.95→0.80，文件小30%
    private static final boolean SLOW_DOWNSCALE_ENABLED = true; // 高清直接降采样，保证播放流畅与写盘速度
    private static final int SLOW_MAX_WIDTH = 640; // 768→640，内存减少30%
    
    // 慢放流停止回调
    private Runnable onSlowMoStreamStopped;
    
    // 实时缓冲区（⭐ 已废弃：现在直接从磁盘读取JPEG，不需要内存缓冲）
    // 设置为1帧占位，避免空指针
    private final com.acard.acard.FrameRingBuffer realtimeBuffer = 
        new com.acard.acard.FrameRingBuffer(1, 320, 180, 0.5f);
    ...
    private void handleZoomScroll(ScrollEvent e) {
        ...
        /*if(player!=null){
            player.setVideoScale((float) proposed);
        }*/
        ...
        Platform.runLater(() -> {
            ...
            try (java.io.InputStream in = getClass().getResourceAsStream(iconPath)) {
                ...
                if (fxImg != null) {
                    ...
                    Tooltip tt = new Tooltip("画质切换(高清/标清)");
                    tt.setShowDelay(javafx.util.Duration.millis(10));
                    btnQualityToggle.setTooltip(tt);
                } else {
    ...
    private void updateCurrentPlayingFrame() {
        try {
            List<com.acard.acard.FrameRingBuffer.FrameItem> snapshot = realtimeBuffer.snapshot();
            if (!snapshot.isEmpty()) {
                // 当前播放帧就是最新推入的帧（缓冲区末尾）
    ...
    public long getLastRealtimeTimestamp() {
        try {
            List<com.acard.acard.FrameRingBuffer.FrameItem> ss = realtimeBuffer.snapshot();
     ...
     * 会等待后向帧到位（直到超时）。
     */
    public com.acard.acard.capture.SnapshotWindowCollector.SnapshotWindowResult<com.acard.acard.FrameRingBuffer.FrameItem> 
    ...
    public com.acard.acard.capture.SnapshotWindowCollector.SnapshotWindowResult<com.acard.acard.FrameRingBuffer.FrameItem> 
        ...
        ArrayList<com.acard.acard.FrameRingBuffer.FrameItem> window =
    ...
    public boolean isDataFlowing() {
        return player != null && player.isDataFlowing();
    }
    ...
    public String getDetailedStatus() {
        if (player != null) {
            return player.getDetailedStatus();
        }
        return "播放器未初始化";
    }
