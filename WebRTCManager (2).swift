...
final class WebRTCManager: NSObject, ObservableObject {
   ...
   private func calculateLadderForDevice(_ device: AVCaptureDevice) {
        // 🔥🔥 统一采集 1920x1440 (4:3)，其他档位通过 scaleResolutionDownBy 缩放
        // ultra (16:9) 是特殊档位，需要单独采集 1280x720
        // 
        // 4:3 档位共用采集：
        //   p4k:      1920x1440 → scaleDown=1.0 → 输出 1920x1440
        //   high:     1920x1440 → scaleDown=1.33 → 输出 1440x1080
        //   standard: 1920x1440 → scaleDown=3.0 → 输出 640x480
        // 
        // 16:9 档位单独采集：
        //   ultra:    1280x720 (后置240fps/前置120fps)
        
        if device.position == .back {
            // 🎥 后置摄像头
            currentLadder = [
                .p4k:      LadderPreset(width: 1920, height: 1440, fps: 60, maxKbps: 5500, maxPushFps: 60, scaleDown: 1.0),      // 4:3 不缩放
                .ultra:    LadderPreset(width: 1280, height: 720, fps: 240, maxKbps: 3500, maxPushFps: 60, scaleDown: 1.0),      // 16:9 单独采集
                .high:     LadderPreset(width: 1440, height: 1080, fps: 60, maxKbps: 3500, maxPushFps: 60, scaleDown: 4.0/3.0),  // 4:3 缩放到1440x1080 (1920÷4/3=1440)
                .standard: LadderPreset(width: 768,  height: 576, fps: 60, maxKbps: 2500, maxPushFps: 60, scaleDown: 2.5)        // 4:3 缩放到768x576
            ]
            print("📐 后置摄像头 - 档位配置（4:3统一采集1920x1440，16:9单独采集）：")
            print("   超高清(p4k)   = 1920x1440 @60fps  → scale=1.0  → 4500kbps")
            print("   超高帧(ultra) = 1280x720  @240fps → scale=1.0  → 3000kbps (16:9单独采集)")
            print("   超清(high)    = 1920x1440 @60fps  → scale=1.33 → 3000kbps (输出1440x1080)")
            print("   超低网(standard)= 1920x1440 @60fps  → scale=3.0  → 1500kbps (输出640x480)")
        } else {
            // 📱 前置摄像头
            currentLadder = [
                .p4k:      LadderPreset(width: 1920, height: 1440, fps: 60, maxKbps: 5500, maxPushFps: 60, scaleDown: 1.0),      // 4:3 不缩放
                .ultra:    LadderPreset(width: 1280, height: 720, fps: 120, maxKbps: 3500, maxPushFps: 60, scaleDown: 1.0),      // 16:9 单独采集
                .high:     LadderPreset(width: 1440, height: 1080, fps: 60, maxKbps: 3500, maxPushFps: 60, scaleDown: 4.0/3.0),  // 4:3 缩放到1440x1080 (1920÷4/3=1440)
                .standard: LadderPreset(width: 768,  height: 576, fps: 60, maxKbps: 2500, maxPushFps: 60, scaleDown: 2.5)        // 4:3 缩放到768x576
            ]
            print("📐 前置摄像头 - 档位配置（4:3统一采集1920x1440，16:9单独采集）：")
            print("   超高清(p4k)   = 1920x1440 @60fps  → scale=1.0  → 4500kbps")
            print("   超高帧(ultra) = 1280x720  @120fps → scale=1.0  → 3000kbps (16:9单独采集)")
            print("   超清(high)    = 1920x1440 @60fps  → scale=1.33 → 3000kbps (输出1440x1080)")
            print("   超低网(standard)= 1920x1440 @60fps  → scale=3.0  → 1500kbps (输出640x480)")
        }
    }
    ...
    // MARK: - 🔥 快门速度控制（cjfps 60-400 直接应用）
    
    /// 设置快门速度（后端下发 cjfps 60-400，直接作为快门速度值）
    /// - Parameter shutterSpeed: 60-400（60=1/60s, 400=1/400s）
    func setCaptureFrameRate(shutterSpeed: Int, forceApply: Bool = false) {
        let oldShutter = cjfpsValue
        // 🔥 限制范围 60-400（后端下发的实际值）
        cjfpsValue = max(60, min(400, shutterSpeed))
    ...
    // 🔥 从登录接口获取推流IP，不再写死
    var srsIP: String {
        return UserDefaults.standard.string(forKey: "stream_push_ip") ?? "8.162.10.80"
    }
    var app   = "tenantA"
    ...
    func applyThinRemoteConfig(_ cfg: ThinRemoteConfig) {
        ...
        case "type":
            ...
            default:
                desiredProfile = .standard
                print("   ⚠️ 未知档位 '\(cfg.type)'，使用默认 .standard")
            }
            ...
            if let f = cfg.fps {
                ...
                // print("🔄 [FPS] \(f)/4=\(f/4) → \(webrtcFps)fps")
        ...
        AVCaptureDevice.requestAccess(for: .video) { granted in
                    DispatchQueue.main.async {
                        ...
                        if let camera = initialCamera {
                            ...
                            // 🔥 获取实际采集分辨率（4:3统一1920x1440，16:9单独1280x720）
    ...
    private func startCaptureWithDevice(_ device: AVCaptureDevice, width: Int, height: Int, fps: Int) {
        ...
        print("🎬 [startCaptureWithDevice] 真实采集分辨率: \(targetWidth)x\(targetHeight)")
        ...
        // 🔥 根据目标帧率选择不同的策略
        let isHighFpsMode = fps > 60  // 高帧率模式（120/240fps）
        ...
        if isHighFpsMode {
            ...
            print("🎯档位🎯 [startCapture] 高帧率模式: 目标\(fps)fps, 找到\(matchingFormats.count)个匹配格式, 其中\(highFpsFormats.count)个支持\(fps)fps+")
            
            if !highFpsFormats.isEmpty {
                candidateFormats = highFpsFormats
                print("🎯档位🎯 ✅ 使用支持\(fps)fps的格式")
            } else {
                candidateFormats = matchingFormats.isEmpty ? allFormats : matchingFormats
                print("🎯档位🎯 ⚠️ 无支持\(fps)fps的格式，使用最接近的格式")
            }
        } else {
            // 🔥 普通模式：优先精确60fps格式
            ...
            print("🎯档位🎯 [startCapture] 普通模式: 目标\(fps)fps, 找到\(matchingFormats.count)个匹配格式 (精确60fps=\(exact60FpsFormats.count)个)")
            ...
            if !exact60FpsFormats.isEmpty {
                ...
                candidateFormats = highFpsFormats
                print("🎯档位🎯 ⚠️ 无精确60fps格式，使用支持\(fps)fps+的格式")
            } else {
                candidateFormats = matchingFormats.isEmpty ? allFormats : matchingFormats
                print("🎯档位🎯 ⚠️ 无\(fps)fps格式，从所有格式中选择")
            }
        }
        ...
        guard let best = candidateFormats.sorted(by: { f0, f1 in
            ...
            let diff0 = abs(max0 - fps)
            let diff1 = abs(max1 - fps)
            return diff0 < diff1
        }).first else { return }
        ...
        print("🎯档位🎯 采集FPS: \(useFps)fps (目标=\(fps), 格式最大=\(maxFps))")
        ...
        //print("🚀 开始采集 (由SDK设置帧率为\(finalFps)fps) - 立即应用横屏方向转换...")
    ...
    private let keyframeIntervalSec: Double = 1.0  // 每1秒强制一个关键帧
   ...
   private let ADAPT_MIN_INTERVAL_SEC: TimeInterval = 8
    ...
    // 🔥 当前采集分辨率（4:3 比例，根据档位动态变化）
    // 不再固定采集，而是根据档位真正采集不同分辨率
    ...
    func applyProfileBitrateOnly(_ p: LadderProfile) {
        ...
        // - 其他 4:3 档位共用 1920x1440，只需改 scaleResolutionDownBy
        let isOldUltra = (oldProfile == .ultra)
        let isNewUltra = (p == .ultra)
        let needRecapture = (isOldUltra != isNewUltra)  // 只有在 4:3 <-> 16:9 切换时才重采集
        
        if needRecapture {
            // 🔥 需要重采集（4:3 <-> 16:9 切换）
            print("   🔄 需要重采集（4:3 ↔ 16:9 切换）")
            if isNewUltra {
                // 切换到 ultra (16:9)
                recaptureWithResolution(width: 1280, height: 720, fps: preset.fps)
            } else {
                // 切换到 4:3 档位，统一采集 1920x1440
                recaptureWithResolution(width: 1920, height: 1440, fps: preset.fps)
            }
        } else {
            // 🔥 同类型切换，只需改 scaleResolutionDownBy
            print("   ✅ 同类型切换，只改缩放比例: \(oldPreset?.scaleDown ?? 1.0) → \(preset.scaleDown)")
            setResolutionScale(preset.scaleDown)
            
            // 更新输出分辨率记录（实际采集不变）
            currentCaptureWidth = isNewUltra ? 1280 : 1920
            currentCaptureHeight = isNewUltra ? 720 : 1440
        }
    ...
    func startPublish(initialProfile: LadderProfile? = nil) {
        ...
        if let serverCfg = ConfigManager.shared.getCurrentConfig() {
            ...
            if let serverFps = serverCfg.fps {
                ...
                let pushFps = serverFps / 4
                targetOutputFPS = pushFps
                print("🎯 [初始化-推流] 从缓存读取FPS: 后端=\(serverFps)fps / 4 = 推送目标=\(pushFps)fps")
            } else {
                    ...
                    pc.setRemoteDescription(.init(type: .answer, sdp: ans)) { err in
                        if let err {
                            ...
                            DispatchQueue.main.async {
                                ...
                                // 🔥🔥 关键修复：推流成功后强制设置 scaleResolutionDownBy = 1.0
                                // 防止 WebRTC 内部自动调整导致分辨率变化
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                                    print("🔧 [推流成功] 强制设置 scaleResolutionDownBy = 1.0")
                                    self.setResolutionScale(1.0)
    ...
    private func findBestFormat(for device: AVCaptureDevice, targetWidth: Int, targetHeight: Int, targetFps: Int) -> AVCaptureDevice.Format? {
        ...
        print("   格式搜索: 找到 \(exactMatches.count) 个精确匹配 \(targetWidth)x\(targetHeight)")
        ...
        if !fpsMatches.isEmpty {
            // 选择最接近目标帧率的（避免选到120/240fps的格式导致功耗过高）
            return fpsMatches.min(by: { f0, f1 in
                let max0 = Int(f0.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                let max1 = Int(f1.videoSupportedFrameRateRanges.map(\.maxFrameRate).max() ?? 0)
                return abs(max0 - targetFps) < abs(max1 - targetFps)
            })
        }
    ...
    */
    ...
    func setMaxBitrateKbps(_ kbps: Int) {
        ...
        if let encoding = verifyParams.encodings.first {
            ...
            print("   分辨率: \(currentCaptureWidth)x\(currentCaptureHeight) (scale=\(verifyScale), 应为1.0)")
            if verifyScale != 1.0 {
                print("   ⚠️ 警告: scaleResolutionDownBy 不是1.0! 实际=\(verifyScale)")
            }
        }
    ...
    /// 🔥 获取档位的实际采集分辨率（4:3统一1920x1440，16:9单独1280x720）
